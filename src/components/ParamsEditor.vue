...
function addParam() {
    ...
    nextTick(() => {
        ...
        const inputs = document.querySelectorAll('[aria-label^="Parameter key"]');
...
function focusNext(idx, type) {
    // If enter is pressed in key, focus value; if in value, focus next key
    const keyInputs = document.querySelectorAll('[aria-label^="Parameter key"]');
    const valueInputs = document.querySelectorAll('[aria-label^="Parameter value"]');
    if (type === 'key' && valueInputs[idx]) {
        valueInputs[idx].focus();
    } else if (type === 'value' && keyInputs[idx + 1]) {
        keyInputs[idx + 1].focus();
    }
}
</script>
